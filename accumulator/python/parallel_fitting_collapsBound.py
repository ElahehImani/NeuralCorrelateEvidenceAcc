...
for region in regions:
    ...
    outpath=infilepath+'_results_compete_collaps/'
    ...
    for n in range(len(onlyfiles)):
        ...
        for t in range(trialNo):
            ...
            u=np.zeros((smpl,2))
            u=np.hstack((np.ones((smpl,1))*right_evidence[t],np.ones((smpl,1))*left_evidence[t]))
        ...
        for param in range(len(init_params)):
            ...
            A[0]=init_params[param]['a1']*np.eye(2)+init_params[param]['a2']*(np.ones((2,2))-np.eye(2))
            rslds.dynamics.As=A
            try:
