...
class _CompoundLinearEmissions(Emissions):
    def __init__(self, N, K, D, M=0, single_subspace=True,
                 N_vec=None, D_vec=None,constantEmission=False, **kwargs):
        """
        ...
        """
        super(_CompoundLinearEmissions, self).__init__(N, K, D, M=M, single_subspace=single_subspace,constantEmission=constantEmission)
        ...
        self.emissions_models = [_LinearEmissions(n, K, d, M) for n, d in zip(N_vec, D_vec)]
        # a = self.Fs
        # self.emissions_models[0].Fs=a
    ...
    def Cs(self, value):
        ...
        for i in range(len(self.N_vec)):
            start=last 
            last = start + self.N_vec[i]
            d_start=d0+1
            d_last=d_start+self.D_vec[i]
            val=value[:,start:last,d_start:d_last]
            self.emissions_models[i].Cs=val
            # val=value[:,start:last,i]
            # self.emissions_models[i].Cs=val[:,:,None]
        # self.emissions_models[0].Cs=value
    ...
    def ds(self, value):
        ...
        for i in range(len(self.N_vec)):
            start=last 
            last = start + self.N_vec[i]
            self.emissions_models[i].ds=value[:,start:last]

    @property
    # def Fs(self):
    #     return np.concatenate([em.Fs for em in self.emissions_models], axis=1)
    def Fs(self):
        fs = np.concatenate([em.Fs for em in self.emissions_models], axis=1)
        k,n,u=fs.shape
        for i in range(k):
            for j in range(n):
                for k in range(u):
...
class _CompoundOrthogonalLinearEmissions(Emissions):
    def __init__(self, N, K, D, M=0, single_subspace=True,
        ...
        # The main purpose of this class is to wrap a bunch of emissions instances
        self.emissions_models = [_OrthogonalLinearEmissions(n, K, d, M) for n, d in zip(N_vec, D_vec)]
        a = self.Fs
        self.emissions_models[0].Fs=a
