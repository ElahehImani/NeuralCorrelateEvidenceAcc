...
class SLDS(object):
    ...
    def __init__(self, N, K, D, *, M=0,
        ...
        dynamics_classes = dict(
            ...
            gaussian_independant=obs.AutoRegressiveObservations_IndependantAcc,
            diagonal_gaussian=obs.AutoRegressiveDiagonalNoiseObservations,
            cognitive_state_gaussian=obs.AutoRegressiveObservations_cognitiveStates,
    ...
    def sample(self, T, input=None, tag=None, prefix=None, with_noise=True):
        ...
        if prefix is None:
            ...
            pi0=np.concatenate(([0.999],(0.001/(K-1))*np.ones(K-1)))
    ...
    def most_likely_states_prob_imani(self,x,data,input=None, mask=None, tag=None):
        pi0 = self.init_state_distn.initial_state_distn
        Ps = self.transitions.transition_matrices(x, input, mask, tag)
        ll = self.dynamics.log_likelihoods(x, input, np.ones_like(x, dtype=bool), tag)
        ll += self.emissions.log_likelihoods(data, input, mask, tag, x)

        discrete_expectations = hmm_expected_states(pi0, Ps, ll)
        return discrete_expectations[0]
    ...
    # #modified by imani
    # def correct_continuous_samples(self,x_samples):
    #     # return x_samples
    #     sw=False
    #     x_samples2=x_samples
    #     if(len(x_samples)==1):
    #         sw=True
    #         x_samples=x_samples[0]

    #     for t in range(len(x_samples)):
    #         if(self.transitions.x1_boundVal==1):
    #             idx=np.where(x_samples[t][:,0]>1)
    #             x_samples[t][idx,0]=1
    #             idx=np.where(x_samples[t][:,0]<0)
    #             x_samples[t][idx,0]=0
    ...
    #         if(self.transitions.x2_boundVal==1):
    #             idx=np.where(x_samples[t][:,1]>1)
    #             x_samples[t][idx,1]=1
    #             idx=np.where(x_samples[t][:,1]<0)
    #             x_samples[t][idx,1]=0
    ...
    def _fit_laplace_em_discrete_state_update(
        ...
        # #modified by imani
        # x_sampless=self.correct_continuous_samples(x_sampless)
    ...
    def _fit_laplace_em_continuous_state_update(self,
        ...
        for (Ez, Ezzp1, _), x0, data, input, mask, tag in \
            ...
            # #modified by imani
            # x_list=[[x]]
            # x_list=self.correct_continuous_samples(x_list)
            # x_list=x_list[0]
            # x=x_list[0]
    ...
    def _fit_laplace_em_params_update(self,
        ...
        # #modified by imani
        # continuous_samples=self.correct_continuous_samples(continuous_samples)
    ...
    def _log_likelihood_imani(self, variational_posterior, datas, inputs):
        total_log_like=0
        for i in range(len(inputs)):
            tag=None
            continuous_samples = variational_posterior.mean_continuous_states[i]
            discrete_samples = variational_posterior.mean_discrete_states[i]
            x_mask = np.ones_like(continuous_samples, dtype=bool)
            mask = np.ones_like(datas[i], dtype=bool)
            log_likes = self.dynamics.log_likelihoods(continuous_samples, inputs[i], x_mask, tag)
            log_likes += self.emissions.log_likelihoods(datas[i], inputs[i], mask, tag, continuous_samples)
            total_log_like += np.sum(discrete_samples * log_likes)

        return total_log_like

    def mean_log_likelihood_imani(self, variational_posterior, datas, inputs):
        total_log_like=0
        for i in range(len(inputs)):
            tag=None
            continuous_samples = variational_posterior.mean_continuous_states[i]
            ...
            log_likes = self.dynamics.log_likelihoods(continuous_samples, inputs[i], x_mask, tag)
            log_likes += self.emissions.log_likelihoods(datas[i], inputs[i], mask, tag, continuous_samples)
            total_log_like += np.mean(discrete_samples * log_likes)
        
        total_log_like=total_log_like/len(inputs)
