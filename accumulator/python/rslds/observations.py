## ----------------------------------------
# this script was taken from the rslds package: https://github.com/davidzoltowski/ssm
# and modified by Elaheh Imani
# You need to clone the main version of the code from
# the above repository and replace this file with the originial one
##
...
class AutoRegressiveObservations_IndependantAcc(_AutoRegressiveObservationsBase):
    """
    ...
    def __init__(self, K, D, M=0, lags=1,
                 ...
                 l2_penalty_V_matrix=[],
                 l2_penalty_A_matrix=[],
                 nu0=1e-4, Psi0=1e-4, learn_V=1, learn_A=1, learnA_state2_3=1, boundX=False):
        super(AutoRegressiveObservations_IndependantAcc, self).\
            __init__(K, D, M, lags=lags)

        self.learn_V = learn_V
        self.learn_A = learn_A
        self.learnA_state2_3=learnA_state2_3
        self.boundX=boundX
        ...
        self.l2_penalty_V_matrix = l2_penalty_V_matrix
        self.l2_penalty_A_matrix = l2_penalty_A_matrix
    ...
    @property
    def params(self):
        return super(AutoRegressiveObservations_IndependantAcc, self).params + (self._sqrt_Sigmas,)

    @params.setter
    def params(self, value):
        self._sqrt_Sigmas = value[-1]
        super(AutoRegressiveObservations_IndependantAcc, self.__class__).params.fset(self, value[:-1])

    def permute(self, perm):
        super(AutoRegressiveObservations_IndependantAcc, self).permute(perm)
        self._sqrt_Sigmas = self._sqrt_Sigmas[perm]
    ...
    def m_step(self, expectations, datas, inputs, masks, tags,
        ...
        # independant accumulators
        for d in range(D):
            if(self.learn_V and self.learn_A):
                ...
                Wk = np.linalg.solve(ExuxuTs_d, ExuyTs_d).T
                ...
                EWxyT =  Wk @ ExuyTs_d
                sqerr = EyyTs[0,d,d] - EWxyT.T - EWxyT + Wk @ ExuxuTs_d @ Wk.T
                nu = self.nu0 + Ens[0]
                Sigmas[0,d,d] = (sqerr + self.Psi0[d,d]) / (nu + D + 1)
            elif(self.learn_A and self.learn_V==0):
                # A_d = E[x_t^2]^{-1} * (E[y_{t} x_{t-1}]  - V*E[x_{t-1} u_t])
                Exx_d = ExuxuTs[0,d,d] 
                Exu_d = ExuxuTs[0,d,d+D]
                Eyx_d = ExuyTs[0,d,d]

                a = 1.0 / Exx_d * (Eyx_d - self.Vs[0,d,d]*Exu_d)
                Wk = np.array([a, self.Vs[0,d,d]])
                
                As[0,d,d] = a
                Vs[0,d,d] = self.Vs[0,d,d]
            ...
            elif(self.learn_A==0 and self.learn_V==1):
                # V_d = E[u_t^2]^{-1} * (E[y_t u_t]  - E[x_{t-1} u_t])
                Euu_d = ExuxuTs[0,d+D,d+D] 
                Exu_d = ExuxuTs[0,d,d+D]
                Eyu_d = ExuyTs[0,[d+D],d]
                beta = 1.0 / Euu_d * (Eyu_d - Exu_d)

                As[0,d,d] = self.As[0,d,d]
                Vs[0,d,d] = beta
                Wk = np.array([1.0, beta])
...
class AutoRegressiveObservations_cognitiveStates(_AutoRegressiveObservationsBase):
    """
    AutoRegressive observation model with Gaussian noise.
    ...
    The parameters are fit via maximum likelihood estimation.
    """
    def __init__(self, K, D, M=0, lags=1,D_vec=[],
                 l2_penalty_A=1e-8,
                 l2_penalty_b=1e-8,
                 l2_penalty_V=1e-8,
                 l2_penalty_V_matrix=[],
                 l2_penalty_A_matrix=[],
                 nu0=1e-4, Psi0=1e-4, learn_V=1, learn_A=1):
        super(AutoRegressiveObservations_cognitiveStates, self).\
            __init__(K, D, M, lags=lags)
    ...
    @property
    def params(self):
        return super(AutoRegressiveObservations_cognitiveStates, self).params + (self._sqrt_Sigmas,)

    @params.setter
    def params(self, value):
        self._sqrt_Sigmas = value[-1]
        super(AutoRegressiveObservations_cognitiveStates, self.__class__).params.fset(self, value[:-1])

    def permute(self, perm):
        super(AutoRegressiveObservations_cognitiveStates, self).permute(perm)
        self._sqrt_Sigmas = self._sqrt_Sigmas[perm]
    ...
    def m_step(self, expectations, datas, inputs, masks, tags,
        ...
        for k in range(K):
            if(k==0):
                state_0=self.D_vec[0]
                a11=ExuxuTs[0,0:state_0,0:state_0]
                a12=ExuxuTs[0,0:state_0,D:]
                a21=ExuxuTs[0,D:,0:state_0]
                a22=ExuxuTs[0,D:,D:]
                ExuxuTs_d=np.vstack((np.hstack((a11,a12)),np.hstack((a21,a22))))
                ExuyTs_d = np.vstack((ExuyTs[0,0:state_0,0:state_0],ExuyTs[0,D:,0:state_0]))

                Wk = np.linalg.solve(ExuxuTs_d, ExuyTs_d).T

                As[0,0:state_0,0:state_0] = Wk[:,0:state_0]
                Vs[0,0:state_0,:] = Wk[:,state_0:state_0+M]
                bs[0,0:state_0]=Wk[:,-1]

                # Solve for the MAP estimate of the covariance
                EWxyT =  Wk @ ExuyTs_d
                sqerr = EyyTs[0,0:state_0,0:state_0] - EWxyT.T - EWxyT + Wk @ ExuxuTs_d @ Wk.T
                nu = self.nu0 + Ens[0]
                Sigmas[0,0:state_0,0:state_0] = (sqerr + self.Psi0[0:state_0,0:state_0]) / (nu + D + 1)

            elif(k==1):
                state_0_1=self.D_vec[0]+self.D_vec[1]

                a11=ExuxuTs[0,0:state_0_1,0:state_0_1]
                a12=ExuxuTs[0,0:state_0_1,D:]
                a21=ExuxuTs[0,D:,0:state_0_1]
                a22=ExuxuTs[0,D:,D:]
                ExuxuTs_d=np.vstack((np.hstack((a11,a12)),np.hstack((a21,a22))))
                ExuyTs_d = np.vstack((ExuyTs[0,0:state_0_1,0:state_0_1],ExuyTs[0,D:,0:state_0_1]))

                Wk = np.linalg.solve(ExuxuTs_d, ExuyTs_d).T

                As[0,0:state_0_1,0:state_0_1] = Wk[:,0:state_0_1]
                Vs[0,0:state_0_1,:] = Wk[:,state_0_1:state_0_1+M]
                bs[0,0:state_0_1]=Wk[:,-1]

                # Solve for the MAP estimate of the covariance
                EWxyT =  Wk @ ExuyTs_d
                sqerr = EyyTs[0,0:state_0_1,0:state_0_1] - EWxyT.T - EWxyT + Wk @ ExuxuTs_d @ Wk.T
                nu = self.nu0 + Ens[0]
                Sigmas[0,0:state_0_1,0:state_0_1] = (sqerr + self.Psi0[0:state_0_1,0:state_0_1]) / (nu + D + 1)

            else:
                state_0_1_2=self.D_vec[0]+self.D_vec[1]+self.D_vec[2]

                a11=ExuxuTs[0,0:state_0_1_2,0:state_0_1_2]
                a12=ExuxuTs[0,0:state_0_1_2,D:]
                a21=ExuxuTs[0,D:,0:state_0_1_2]
                a22=ExuxuTs[0,D:,D:]
                ExuxuTs_d=np.vstack((np.hstack((a11,a12)),np.hstack((a21,a22))))
                ExuyTs_d = np.vstack((ExuyTs[0,0:state_0_1_2,0:state_0_1_2],ExuyTs[0,D:,0:state_0_1_2]))

                Wk = np.linalg.solve(ExuxuTs_d, ExuyTs_d).T

                As[0,0:state_0_1_2,0:state_0_1_2] = Wk[:,0:state_0_1_2]
                Vs[0,0:state_0_1_2,:] = Wk[:,state_0_1_2:state_0_1_2+M]
                bs[0,0:state_0_1_2]=Wk[:,-1]

                # Solve for the MAP estimate of the covariance
                EWxyT =  Wk @ ExuyTs_d
                sqerr = EyyTs[0,0:state_0_1_2,0:state_0_1_2] - EWxyT.T - EWxyT + Wk @ ExuxuTs_d @ Wk.T
                nu = self.nu0 + Ens[0]
                Sigmas[0,0:state_0_1_2,0:state_0_1_2] = (sqerr + self.Psi0[0:state_0_1_2,0:state_0_1_2]) / (nu + D + 1)
...
class AutoRegressiveObservations(_AutoRegressiveObservationsBase):
    ...
    def __init__(self, K, D, M=0, lags=1,
                 ...
                 l2_penalty_b=1e-8,
                 l2_penalty_V=1e-8,
                 nu0=1e-4, Psi0=1e-4,learn_V=1, learn_A=1, learnA_state2_3=1):
...
# -------------------- dont learn b --------------------------------------------------
        k=0
        #remove b
        if self.learn_V==1 and self.learn_A==1:
            ExuxuTs_tmp = ExuxuTs[:,:D*lags+M,:D*lags+M]
            ExuyTs_tmp = ExuyTs[:,:D*lags+M,:]
            Wk = np.linalg.solve(ExuxuTs_tmp[k] + self.J0[k,:D*lags+M,:D*lags+M], ExuyTs_tmp[k] + self.h0[k,:D*lags+M,:]).T
            # Wk = np.linalg.solve(ExuxuTs[k], ExuyTs[k]).T
            As[k] = Wk[:, :D * lags]
            Vs[k] = Wk[:, D * lags:D * lags+M]

        elif self.learn_A==1 and self.learn_V==0:
            ExuxuTs_tmp = ExuxuTs[:,:D*lags,:D*lags]
            ExuyTs_tmp = ExuyTs[:,:D*lags,:]
            Wk = np.linalg.solve(ExuxuTs_tmp[k] + self.J0[k,:D*lags,:D*lags], ExuyTs_tmp[k] + self.h0[k,:D*lags,:D*lags]).T
            As[k] = Wk[:, :D * lags]
            Vs[k] = self.Vs[k]

        # elif self.learn_V==1 and self.learn_A==0:
        #     ExuxuTs_tmp = ExuxuTs[:, D*lags:D*lags+M, D*lags:D*lags+M]
        #     ExuyTs_tmp = ExuyTs[:,D*lags:D*lags+M,:]
        #     X=ExuxuTs_tmp[k] + self.J0[:, D*lags:D*lags+M, D*lags:D*lags+M]
        #     Y=ExuyTs_tmp[k] + self.h0[k, D*lags:D*lags+M, :]
        ...
        # Solve for the MAP estimate of the covariance
        EWxyT =  Wk @ ExuyTs_tmp[k]
        sqerr = EyyTs[k] - EWxyT.T - EWxyT + Wk @ ExuxuTs_tmp[k] @ Wk.T
        nu = self.nu0 + Ens[k]
        Sigmas[k] = (sqerr + self.Psi0) / (nu + D + 1)

        if(self.learnA_state2_3):
            #remove V and b
            ExuxuTs = ExuxuTs[:,:D * lags,:D * lags]
            ExuyTs = ExuyTs[:,:D * lags,:]
            J0 = self.J0[:,:D * lags,:D * lags]
            h0 = self.h0[:,:D * lags,:D * lags]
...
class AutoRegressiveObservations_Independant(_AutoRegressiveObservationsBase):
    """
    AutoRegressive observation model with Gaussian noise.
...
# -------------------- dont learn b --------------------------------------------------
        k=0
        #remove b
        ExuxuTs = ExuxuTs[:,:D*lags+M,:D*lags+M]
        ExuyTs = ExuyTs[:,:D*lags+M,:]

        # first accumulator
        dd=D/2
        Wk = np.linalg.solve(ExuxuTs[k] + self.J0[k,:D*lags+M,:D*lags+M], ExuyTs[k] + self.h0[k,:D*lags+M,:D*lags+M]).T
        As[k] = Wk[:, :D * lags]
        Vs[k] = Wk[:, D * lags:D * lags+M]
